...
impl<T: StreamProcessor> StreamRunner<T> {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        let mut msg_stream = consumer.stream().map(|x| -> Result<T::Output> {
            ...
            let output = self.processor.handle_message(deserialized);
            Ok(output)
        });
        while let Some(msg) = msg_stream.next().await {
            ...
            match res {
                Ok((partition, offset)) => trace!(
                    "Message successfully delivered to partition {}, offset {}",
                    partition,
                    offset
                ),
...
mod tests {
    ...
    impl StreamProcessor for EchoProcessor {
        ...
        fn handle_message(&self, input: f32) -> f32 {
            input
        }
